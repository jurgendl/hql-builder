 ...
 * ZussResourceReference(WicketCSSRoot.class, "table.css")));
 */
public class ZussResourceReference extends StreamResourceReference implements
		IResourceStream, Resolver {
	private static final long serialVersionUID = 6384603768717480808L;

	protected static final Logger logger = LoggerFactory
			.getLogger(ZussResourceReference.class);
	...
	public Time lastModifiedTime() {
		...
		logger.info(getZussName() + " - sending lastModifiedTime: "
				+ lastModified);
		return lastModified;
	}
	...
	public InputStream getInputStream() throws ResourceStreamNotFoundException {
		try {
			...
			if (css == null) {
				...
				rebuild = true;
			} else if (lastModified.getMilliseconds() < getZussStyle()
					.getLastModified()) {
	...
	protected InputStream read() throws IOException {
		return getClass().getClassLoader().getResourceAsStream(
				getResourcePath() + '/' + getZussName());
	}

	protected void write(OutputStream out) throws IOException {
		ZussDefinition parsed = Zuss
				.parse(read(), charset, null, getZussName());
	...
	protected Resolver getResolver() {
		if (resolver == null) {
			resolver = new BuiltinResolver() {
				@Override
				public Object getVariable(String name) {
					Object variable = ZussResourceReference.this
							.getVariable(name);
					if (variable == null) {
