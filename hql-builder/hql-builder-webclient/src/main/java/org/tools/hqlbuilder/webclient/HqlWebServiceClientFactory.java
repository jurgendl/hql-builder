...
public abstract class HqlWebServiceClientFactory<R> implements MethodHandler,
		InitializingBean {
	...
	protected Class<R> setupResourceClass() {
		return (Class<R>) ((java.lang.reflect.ParameterizedType) getClass()
		...
		if (this.resteasyProvider == null) {
			if (restEasyProvidersBean != null) {
				...
				throw new IllegalArgumentException(
						"either packages or restEasyProvidersBean is required");
			}
		}
	...
	@SuppressWarnings("deprecation")
	@Override
	public Object invoke(Object self, Method method, Method forwarder,
		...
		UriBuilder uriBuilder = UriBuilder.fromUri(URI.create(getServiceUrl()))
				.path(getResourceClass()).path(method);
		List<Object> notAcceptedParameters = new ArrayList<Object>(
				Arrays.asList(args));
		Map<String, Object> templateValues = new HashMap<String, Object>();
		for (int i = 0; i < parameterTypes.length; i++) {
			logger.debug("parameter: " + parameterTypes[i].getName() + " "
					+ args[i]);
			logger.debug("parameter-annotations: "
					+ Arrays.toString(parameterAnnotations[i]));
			for (Annotation parameterAnnotation : parameterAnnotations[i]) {
				if (parameterAnnotation instanceof QueryParam) {
					QueryParam queryParam = QueryParam.class
							.cast(parameterAnnotation);
				...
				if (parameterAnnotation instanceof PathParam) {
					PathParam pathParam = PathParam.class
							.cast(parameterAnnotation);
		...
		org.jboss.resteasy.client.ClientRequest request = new org.jboss.resteasy.client.ClientRequest(
				uriBuilder, getClientExecutor(), getResteasyProvider());
		...
		for (int i = 0; i < parameterTypes.length; i++) {
			...
			for (Annotation parameterAnnotation : parameterAnnotations[i]) {
				...
				if (parameterAnnotation instanceof MatrixParam) {
					MatrixParam matrixParam = MatrixParam.class
							.cast(parameterAnnotation);
				...
				if (parameterAnnotation instanceof FormParam) {
					FormParam formParam = FormParam.class
							.cast(parameterAnnotation);
				...
				if (parameterAnnotation instanceof CookieParam) {
					CookieParam cookieParam = CookieParam.class
							.cast(parameterAnnotation);
				...
				if (parameterAnnotation instanceof HeaderParam) {
					HeaderParam headerParam = HeaderParam.class
							.cast(parameterAnnotation);
		...
		try {
			...
			if (stream) {
				entity = new RedirectedStreamingOutput(
	...
	public void setRestEasyProvidersBean(
			RestEasyProvidersBean restEasyProvidersBean) {
