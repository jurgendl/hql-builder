...
public class HqlWebServiceClientTest {
	public static void main(String[] args) {
		try {
			...
			System.out.println(hc
					.findParameters("select version from Pojo where id=:id"));
			System.out.println(hc.execute(new QueryParameters(
					"select version from Pojo where id=:id", -1,
					new QueryParameter("id", "1l"))));
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
