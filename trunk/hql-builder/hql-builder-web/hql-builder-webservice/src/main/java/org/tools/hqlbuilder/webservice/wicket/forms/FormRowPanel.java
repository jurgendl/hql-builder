...
public abstract class FormRowPanel<P, T, C extends FormComponent<T>, S extends FormElementSettings>
		extends Panel implements FormConstants {
	private static final long serialVersionUID = 5258950770053560483L;

	protected static final Logger logger = LoggerFactory
			.getLogger(FormRowPanel.class);
	...
	public FormRowPanel(P propertyPath, IModel<T> valueModel,
			FormSettings formSettings, S componentSettings) {
		this(valueModel, propertyPath, formSettings, componentSettings);
		this.valueModel = valueModel;
	}

	protected FormRowPanel(IModel<?> model, P propertyPath,
			FormSettings formSettings, S componentSettings) {
	...
	protected void setupRequiredBehavior() {
		C c = getComponent();
		if (formSettings.isAjax()
				&& formSettings.isLiveValidation()
				&& !(c instanceof PasswordTextField)
			...
			label = new Label(LABEL, getLabelModel()) {
				...
				public boolean isVisible() {
					return super.isVisible()
							&& (formSettings == null || formSettings
									.isShowLabel());
				}
	...
	protected FeedbackPanel getFeedback() {
		if (feedbackPanel == null) {
			feedbackPanel = new FeedbackPanel(FEEDBACK_ID,
					new ComponentFeedbackMessageFilter(component)) {
	...
	protected WebMarkupContainer getRequiredMarker() {
		WebMarkupContainer requiredMarker = new WebMarkupContainer(
				"requiredMarker") {
	...
	protected void setupPlaceholder(ComponentTag tag) {
		if ((formSettings != null && formSettings.isShowPlaceholder())
				|| (componentSettings != null && componentSettings
						.isShowPlaceholder())) {
	...
	protected void setupRequired(ComponentTag tag) {
		if (formSettings != null
				&& formSettings.isClientsideRequiredValidation()
				&& componentSettings != null && componentSettings.isRequired()) {
	...
	protected void setupRequired(C component) {
		...
		if (StringUtils.isNotBlank(formSettings.getRequiredClass())) {
			if (componentSettings.isRequired()) {
				component.add(new CssClassNameAppender(formSettings
						.getRequiredClass()));
			} else {
				component.add(new CssClassNameRemover(formSettings
						.getRequiredClass()));
			}
		}
	}
	...
	public String getPropertyName() {
		if (propertyName == null) {
			try {
				...
				propertyName = propertyPath == null ? null : propertyPath
						.toString();
			}
		}
	...
	public FormRowPanel<P, T, C, S> inheritId() {
		...
		getComponent().setMarkupId(
				getPropertyName().toString().replace('.', DOT_REPLACER));
		return this;
	}
}
