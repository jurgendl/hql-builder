...
public class PropertyPanel extends PropertySheetPanel {
    ...
    private final IHqlBuilderHelper helper;
    ...
    public PropertyPanel(final SessionFactory sessionFactory, Object bean, final boolean editable, IHqlBuilderHelper helper) {
        this.helper = helper;
        Session session = null;
        try {
            if (bean != null && sessionFactory != null) {
        ...
        for (final Map.Entry<String, PropertyDescriptor> propertyDescriptorEntry : propertyDescriptors.entrySet()) {
            try {
                helper.log(propertyDescriptorEntry.getKey());
                    ...
                    if (writeMethod != null) {
                        if (null == propertyEditorRegistry.getEditor(propertyType) && !String.class.equals(propertyType)) {
                            ...
                            if (Object[].class.isAssignableFrom(propertyType) || Collection.class.isAssignableFrom(propertyType)
                                ...
                                helper.log("no editor found for type " + propertyType.getName());
                            }
                            property.setEditable(false);
                        } else {
                            ...
                            property.addPropertyChangeListener(new PropertyListener(sessionFactory, writeMethod));
                        }
                    }
                }
    ...
    private class PropertyListener implements PropertyChangeListener {
        ...
        public PropertyListener(SessionFactory sessionFactory, Method writeMethod) {
        ...
        public void propertyChange(final PropertyChangeEvent evt) {
            ...
            try {
                if (!new EqualsBuilder().append(evt.getOldValue(), evt.getNewValue()).isEquals()) {
                    helper.log("changing value from '" + evt.getOldValue() + "' to '" + evt.getNewValue() + "'");
